...
- **Date Verified**: February 16, 2026
...
- [x] Scrolling logo watermark background on authenticated pages only
- [x] Background pattern uses XOROOTS logo with low opacity (0.03 light, 0.05 dark)
- [x] Background scrolls continuously in diagonal direction
- [x] Background is non-interactive (pointer-events: none)
...
- [x] Scrolling background adapts opacity for light/dark themes
...
- [x] Scrolling background references logo via CSS url() from static path

### Code Quality ✓
- [x] No hardcoded logo paths outside branding constants and CSS
...
- [x] Hidden on landing page (logged-out state)
- [x] Positioned behind all content (z-index: 0)
- [x] Does not interfere with user interactions
- [x] Continuous diagonal scrolling animation (30s loop)
- [x] Subtle opacity maintains text readability
...
- The scrolling logo background adds subtle brand reinforcement without distracting from content
