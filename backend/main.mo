...
actor {
  ...
  public query ({ caller }) func getCallerUserProfile() : async ?CoachProfile {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
  ...
  public query ({ caller }) func initializeCallerProfile() : async ?CoachProfile {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
  ...
  public shared ({ caller }) func deleteJobPosting(jobId : Text) : async () {
    ...
    switch (jobPosts.get(jobId)) {
      ...
      case (?job) {
        // Users can delete their own job postings, admins can delete any job posting
        if (job.poster != caller and not AccessControl.isAdmin(accessControlState, caller)) {
